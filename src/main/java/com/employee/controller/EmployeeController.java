...
public class EmployeeController {
	...
	@ApiResponses(value = {@ApiResponse(code = 200, message = "Role is successfully deleted."),  @ApiResponse(code = 400, message = CommonConstants.MESSAGE_400,response = ErrorMessage.class), @ApiResponse(code = 500, message = CommonConstants.MESSAGE_500,response = ErrorMessage.class)})
	...
	@ApiResponses(value = {@ApiResponse(code = 200, message = "Any field level validation error OR business logic error"),  @ApiResponse(code = 400, message = CommonConstants.MESSAGE_400,response=ErrorMessage.class), @ApiResponse(code = 500, message = CommonConstants.MESSAGE_500,response=ErrorMessage.class)})
	...
	@ApiOperation(value = "Update's an existing employee", notes="API will update employee.")
    @ApiResponses(value = {@ApiResponse(code = 200, message = "Employee has been updated successfully."),  @ApiResponse(code = 400, message = CommonConstants.MESSAGE_400,response=ErrorMessage.class), @ApiResponse(code = 500, message = CommonConstants.MESSAGE_500,response=ErrorMessage.class)})
	public void updateEmployee( @PathVariable(name="empId",required=true) int empId, @Valid @RequestBody Employee emp ) {
